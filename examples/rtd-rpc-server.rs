...
enum PossibleCommand {
    // 'S,?' command, we query the current temperature scale of the sensor chip.
    Scale,
    // 'R'
    Read,
    // 'SLEEP' command
...
fn parse_cli_arguments() -> Result<()> {
    let matches = App::new("benita-rtd-rep")
        .version("0.1.0")
        .author("Joaquin R. <globojorro@gmail.com>")
        .about("Benita IoT. A response service for temperature data.")
        .arg(
